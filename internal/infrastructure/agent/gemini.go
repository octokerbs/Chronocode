...
func (ga *GeminiAgent) AnalyzeCommitDiff(ctx context.Context, diff string) (domain.CommitAnalysis, error) {
	tries := 3
	...
	if err != nil {
		return domain.CommitAnalysis{}, errors.NewError(errors.ErrInternalFailure, err)
	}

	var commitAnalysis domain.CommitAnalysis
