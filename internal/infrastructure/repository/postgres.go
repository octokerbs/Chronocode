...
import (
	...
	"github.com/octokerbs/chronocode-backend/pkg/errs"
)
...
func NewPostgresClient(dsn string) (*PostgresClient, error) {
	if dsn == "" {
		return nil, errs.NewError(errs.ErrBadRequest, errors.New("repository connection string is empty"))
	}
	
	db, err := sql.Open("postgres", dsn)
	if err != nil {
		return nil, errs.NewError(errs.ErrInternalFailure, err)
	}
