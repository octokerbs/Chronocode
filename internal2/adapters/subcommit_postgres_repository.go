...
func NewPostgresSubcommitRepository(postgresClient *sql.DB) (*PostgresSubcommitRepository, error) {
	if postgresClient == nil {
		return nil, errors.New("missing postgresClient")
	}

	return &PostgresSubcommitRepository{db: postgresClient}, nil
}

func (pg *PostgresSubcommitRepository) GetRepository(ctx context.Context, id int64) (*analysis.Repository, error) {
	const query = `
...
func (pg *PostgresSubcommitRepository) StoreRepository(ctx context.Context, repo *analysis.Repository) error {
	const upsertQuery = `
...
func (pg *PostgresSubcommitRepository) StoreCommit(ctx context.Context, commit *analysis.Commit) error {
	if commit == nil {
		return nil
	}
...
func (pg *PostgresSubcommitRepository) storeSubcommits(tx *sql.Tx, subcommits []*analysis.Subcommit) error {
	if len(subcommits) == 0 {
		return nil
	}
...
func (pg *PostgresSubcommitRepository) GetSubcommitsByRepoID(ctx context.Context, repoID int64) ([]*analysis.Subcommit, error) {
